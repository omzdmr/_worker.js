// Pages üzerinden deploy önerilir (Worker'a göre daha stabil routing)
...
async function 启动传输管道(WS接口) {
  let TCP接口, 识别地址类型, 访问地址, 地址长度, 首包数据 = false, 首包处理完成 = null, 传输数据, 读取数据, 传输队列 = Promise.resolve();
  try {
    ...
    async function 解析首包数据(首包数据) {
      ...
      if (写入初始数据.length > 0) {
        try { await 传输数据.write(写入初始数据) } catch (e) { throw (e) };
      }
